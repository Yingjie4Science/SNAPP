...
<sub>Figure. Annual modeled preventable depression cases under four national greening counterfactuals plus the existing-greenness accounting reference. The p95 scenario is an aspirational upper bound; existing greenness is not an investment program.</sub>
...
| Existing greenness (accounting) | Compare observed NDVI with an NDVI=0 accounting reference; not an investment scenario. | 12,733,902 | 58.24 | $270,977,433,363 | 10.07 |

<sub>Table legend. Values are annual central estimates. All rows use 218,643,229 ACS adults and exactly 1,167 counties. These scenarios differ in greening magnitude and feasibility, so they should not be ranked as equal-budget policy alternatives. Existing greenness is a stock-value accounting counterfactual and must be interpreted separately.</sub>

...
county from the ACS 5-year API and writes:
    ...
    config/adult_population.csv -> GEOID, name, population_total,
                                   population_adult, adult_fraction

run_city.py reads that file automatically, uses the fraction, and calibrates
WorldPop's aggregate to the ACS adult count while preserving its spatial pattern.
...
USAGE
    ...
    python src/inputs/fetch_adult_fraction.py --regions data/national/counties.gpkg
...
OUT_CSV = BASE_DIR / "config" / "adult_population.csv"
...
def main():
    ...
    ap.add_argument("--regions", type=Path,
                    help="Optional county AOI; when supplied, output only its GEOIDs "
                         "and fail if any are absent from ACS.")
    ap.add_argument("--out", type=Path, default=OUT_CSV)
    cli = ap.parse_args()

    url = f"https://api.census.gov/data/{cli.year}/acs/acs5"
    params = {"get": f"NAME,{ADULT_VAR},{TOTAL_VAR}", "for": "county:*"}
    key = cli.api_key or os.getenv("CENSUS_API_KEY")
    if key:
        params["key"] = key
    ...
    try:
        ...
        sys.exit(f"Census ACS request failed: {exc}")
    ...
    for row in rows:
        ...
        out_rows.append({
            ...
            "name": name,
            "acs_year": cli.year,
            "population_total": round(total),
            "population_adult": round(adults),
            "adult_fraction": round(frac, 8),
        })
    ...
    if cli.regions:
        ...
        if "GEOID" not in regions.columns:
            sys.exit(f"No GEOID column in {cli.regions}.")
        expected = set(regions["GEOID"].astype(str).str.zfill(5))
        out_rows = [row for row in out_rows if row["GEOID"] in expected]
        missing = sorted(expected - {row["GEOID"] for row in out_rows})
        if missing:
            sys.exit(f"ACS lookup is missing {len(missing)} AOI counties: {missing}")
    cli.out.parent.mkdir(parents=True, exist_ok=True)
    with open(cli.out, "w", newline="") as fh:
        w = csv.DictWriter(
            fh, fieldnames=["GEOID", "name", "acs_year", "population_total",
                            "population_adult", "adult_fraction"],
            lineterminator="\n")
        w.writeheader()
